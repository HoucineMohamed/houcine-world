...
import { buildAuthorizeUrl, exchangeCodeForToken, fetchChannel, YouTubeApiError, YOUTUBE_SCOPES } from "../_shared/youtube.ts";

/**
 * Google OAuth2 (YouTube Data API v3), same shape as spotify-oauth:
 *   POST …/youtube-oauth/authorize  { brand_id }  Authorization: Bearer <user JWT>  -> { url }
 *   GET  …/youtube-oauth/callback?code=&state=    (Google calls this directly)     -> 302 redirect
 */
...
const handleAuthorize = async (req: Request) => {
  ...
  if (!creds) {
    return json({ error: "YouTube isn't configured yet. Ask a super admin to set it up under Platform Credentials." }, 409);
  }
  ...
  if (insertError) return json({ error: "Could not start the YouTube connection" }, 500);
...
const handleCallback = async (req: Request) => {
  ...
  if (url.searchParams.get("error")) return redirectToSettings(origin, brandId, { youtube: "denied" });

  if (!stateUsable) {
    return redirectToSettings(origin, brandId, { youtube: "error", reason: "invalid_state" });
  }

  const code = url.searchParams.get("code");
  if (!code) return redirectToSettings(origin, brandId, { youtube: "error", reason: "missing_code" });

  try {
    const creds = await getPlatformCredentials(svc, "youtube");
    if (!creds) return redirectToSettings(origin, brandId, { youtube: "error", reason: "config_error" });

    const token = await exchangeCodeForToken(creds, code, redirectUriFor("youtube"));
    if (!token.refresh_token) throw new YouTubeApiError("Google did not return a refresh token", "api_error");
    ...
    const { data: account, error: upsertError } = await svc
      ...
      .select("id")
      .single();
    if (upsertError || !account) throw new YouTubeApiError("Could not save the connection", "api_error");
    ...
    await svc.from("connected_account_secrets").upsert(
      {
        ...
        provider_meta: { youtube_channel_id: channel.id, uploads_playlist_id: channel.uploadsPlaylistId },
      },
    ...
    await svc.from("audit_logs").insert({
      ...
      metadata: { platform: "youtube", account_name: channel.title },
    });

    return redirectToSettings(origin, brandId, { youtube: "connected" });
  } catch (e) {
    console.error("youtube-oauth callback failed:", e);
    const reason = e instanceof YouTubeApiError ? e.reason : "unknown";
    return redirectToSettings(origin, brandId, { youtube: "error", reason });
  }
};
