...
import { Users, Loader2, AlertTriangle, Building2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { fetchMetaBusinessMetrics, type MetaBusinessMetricsResponse } from "@/services/connectedAccounts";

/**
 * Meta Business Suite's own metrics card — same rendering as the generic
 * PlatformMetricsCard (kept untouched), but calling fetchMetaBusinessMetrics
 * directly instead of the generic `${platform}-metrics` helper, since this
 * platform's edge-function route ("meta-business-metrics") doesn't match its
 * stored `platform` value ("meta_business"). Renders nothing until the
 * brand has connected this platform — connecting lives in Settings.
 */
const MetaBusinessMetricsCard = ({ brandId }: { brandId: string }) => {
  const [data, setData] = useState<MetaBusinessMetricsResponse | null>(null);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setFailed(false);
    fetchMetaBusinessMetrics(brandId)
    ...
    <Card className="bg-card border-border mt-6">
      <CardHeader className="flex flex-row items-center justify-between gap-4 space-y-0">
        <CardTitle className="text-base flex items-center gap-2">
          <Building2 className="w-4 h-4 text-accent" /> Meta Business Suite
      ...
      <CardContent>
        {needsReconnect && (
          <div className="mb-4 flex flex-wrap items-center gap-2 rounded-lg border border-destructive/40 bg-destructive/5 px-3 py-2 text-sm text-destructive">
            ...
            <span className="flex-1">{data.message ?? "Meta Business Suite needs to be reconnected."}</span>
        ...
        {!metrics ? (
          <p className="text-sm text-muted-foreground py-6 text-center">No Meta Business Suite data yet.</p>
        ) : (
...
export default MetaBusinessMetricsCard;
